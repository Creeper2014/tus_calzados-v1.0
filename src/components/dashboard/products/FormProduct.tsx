...
export const FormProduct = ({ titleForm }: Props) => {
  ...
  const navigate = useNavigate();
  	...
  	useEffect(() => {
		if (product && !isLoading) {
			...
			// ✅ Aquí agregamos el valor de free_shipping
    		setValue('free_shipping', product.free_shipping || false);
			setValue('type', product.type); //gpt
		}
  	...
  	const onSubmit = handleSubmit(data => {
		...
		if (slug) {
			updateProduct({
				...
				features,
				type: data.type,
  				free_shipping: data.free_shipping ?? false, // ✅ asegura boolean
  ...
  const watchName = watch('name');
	...
	useEffect(() => {
		...
		const currentSlug = watch('slug'); ///gpt

		if(!currentSlug) { //EL IF TAMBIEN ES DE GPT
      	...
      	<div className='flex justify-between items-center'>
			...
			>
				...
				>
					<InputForm 
						type='text'
						placeholder='Ejemplo: Nike Air Max 200'
						label='nombre'
						name='name'
						register={register}
						errors={errors}
				...
				<SectionFormProduct>
					...
					<InputForm
						...
						label='Tipo'
						name='type'
						placeholder='Remera, Jeans, Zapatilla...'
						register={register}
						errors={errors}
						required
					/>
					...
					<InputForm
						type='checkbox'
						label='Envío Gratis'
						name='free_shipping' // campo en Supabase
						register={register}
						errors={errors}
					/>
